...
void ThreatsObj::CheckToMap(Map& map_data)
{
    ...
    if (x1 >= 0 && x2 < MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y)
    {
        if (x_val_ > 0) //mainobj đang di chuyển sang phải
        {
            ...
            if ((val1 != BLANK_TILE && val1 != STATE_ITEM1 && val1 != STATE_ITEM2 && val1 != STATE_ITEM3) || (val2 != BLANK_TILE && val2 != STATE_ITEM1 && val2 != STATE_ITEM2 && val2 != STATE_ITEM3))
            {
                x_pos_ = x2*TILE_SIZE;
    ...
    if (y_pos_+65 > map_data.max_y_) // xử lí lỗi 1 vài hố rơi không chết (+1 tile map)
    {
        come_back_time = 60;
    }
